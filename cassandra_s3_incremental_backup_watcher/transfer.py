...
class TransferManager(BaseSubscriber):
    ...
    def schedule(self, sstables, check_remote=True):
        ...
        # Get information about existing objects to avoid duplicate uploads
        existing_objects = {}
        if check_remote:
            dest_paths = []
            for _, _, dest_path in self._sstable_paths(sstable):
                dest_paths.append(dest_path)

            prefix = os.path.commonprefix(dest_paths)
